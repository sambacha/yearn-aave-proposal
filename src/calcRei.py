...
class FinancialSystem(object):
    ...
    def __init__(
        ...
        self.d = np.array(
            [
                self.D[self.node2ind[elem[0]], self.node2ind[elem[1]]]
                for elem in self.H.nodes()
            ]
        )
        self.c = np.array(
            [
                self.C[self.node2ind[elem[0]], self.node2ind[elem[1]]]
                for elem in self.H.nodes()
            ]
        )
        self.gamma = sparse.diags(
            [
                self.Gamma[self.node2ind[elem[0]], self.node2ind[elem[1]]]
                for elem in self.H.nodes()
            ]
        )
    ...
    def rand_primary_prems(self, prlb, prub):
        ...
        prems_reins = dict(
            self.Gprems.in_degree(weight="weight")
        )  # reinsurance premiums received
        prems_ceded = dict(
            self.Gprems.out_degree(weight="weight")
    ...
    def rand_foreign_reins_prems(self, rrlb, rrub):
        ...
        for node in self.Gprems.nodes():
            if (
                prems_reins[node] != 0
                and prems_ceded[node] / prems_reins[node] > self.rrub
            ):
    ...
    def rand_node_capital(self, lrlb, lrub):
        ...
        for node in self.Gprems.nodes():
            ...
            prems_net = (
                prems_reins[node]
                + self.prems_f_reins[node]
                + self.prems_prim[node]
                - prems_ceded[node]
            )
...
def solve_no_caps(G, D, sh):
    ...
    while finish == False:
        ell = np.linalg.solve(
            (np.eye(m) - np.dot(gamma, np.dot(np.diag(b_0), X))),
...
def solve_caps(G, D, C, sh):
    ...
    while finish == False:
        ...
        tell = np.linalg.solve(
            (np.eye(tm) - np.dot(tgamma, np.dot(tB, tX))),
...
def solve_caps_combo(fs, illcond=2e3):
    ...
    while finish == False:
        if inv_error == False:
            try:
                ell, B_1, C_1 = iterate_capped_lsystem(
                    fs, B_0, C_0, last_ell, illcond=illcond
                )
                last_ell = ell[:]
            ...
            if B_1 != B_0 or C_1 != C_0:
                ...
                L_0 - L_1
            ).nnz == 0:  # might want to replace with a convergence threshold
                finish = True
                return L_1, B_1, C_1
...
def solve_caps_combo_back(fs, illcond=2e3):
    ...
    while finish == False:
        if inv_error == False:
            try:
                ell_1, B_1, C_1 = iterate_capped_lsystem_back(
                    fs, B_0, C_0, illcond=illcond
                )
                ell_0 = ell_1[:]
...
def sp_cond_num(M):
    ...
    eig_large = sparse.linalg.eigsh(
        A, k=1, which="LM", tol=1e-3, return_eigenvectors=False
    )
    eig_small = sparse.linalg.eigsh(
        A, k=1, sigma=0, which="LM", tol=1e-3, return_eigenvectors=False
    )
